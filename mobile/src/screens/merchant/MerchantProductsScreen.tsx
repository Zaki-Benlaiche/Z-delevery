...
export function MerchantProductsScreen() {
  ...
  const add = useMutation({
    mutationFn: () =>
      merchantsApi.addProduct(merchantId!, { name: name.trim(), price: parseFloat(price.replace(",", ".")), description: desc.trim() || null }),
    onSuccess: () => {
      invalidate();
      setAdding(false);
      setName(""); setPrice(""); setDesc("");
    },
    onError: (e) => Alert.alert("تعذّر الإضافة", (e as Error).message),
  });
  ...
  return (
    <Screen padded={false}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !store.isLoading ? <EmptyState icon="🍽️" title="لا منتجات بعد" hint="أضف أوّل منتج ليظهر للزبائن" ctaLabel="إضافة منتج" onCta={() => setAdding(true)} /> : null
        }
        renderItem={({ item }) => (
          <ProductRow product={item} onToggle={(a) => toggle.mutate({ id: item.id, available: a })} onDelete={() =>
      ...
      <Modal visible={adding} transparent animationType="slide" onRequestClose={() => setAdding(false)}>
        <View style={styles.overlay}>
          <View style={styles.sheet}>
            ...
            <Text style={styles.sheetTitle}>منتج جديد</Text>
            <Input label="اسم المنتج" value={name} onChangeText={setName} placeholder="مثال: بيتزا مارغريتا" />
            <Input label="السعر (دج)" value={price} onChangeText={setPrice} keyboardType="decimal-pad" placeholder="مثال: 800" />
            <Input label="وصف (اختياري)" value={desc} onChangeText={setDesc} placeholder="مكوّنات أو تفاصيل" />
            <Button label="حفظ المنتج" onPress={submitAdd} loading={add.isPending} size="lg" />
            <Button label="إلغاء" variant="ghost" onPress={() => setAdding(false)} />
          </View>
        </View>
      </Modal>
    </Screen>
...
function ProductRow({ product, onToggle, onDelete }: { product: Product; onToggle: (a: boolean) => void; onDelete: () => void }) {
  return (
