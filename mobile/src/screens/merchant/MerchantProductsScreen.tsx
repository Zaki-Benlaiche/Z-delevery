...
export function MerchantProductsScreen() {
  ...
  const save = useMutation({
    mutationFn: () => {
      const payload = { name: name.trim(), price: parseFloat(price.replace(",", ".")), description: desc.trim() || null, category: category.trim() || null, image_url: image };
      return editing
  ...
  return (
    <Screen padded={false}>
      ...
      <MerchantSettingsScreen visible={settingsOpen} onClose={() => setSettingsOpen(false)} />
      ...
      <Modal visible={modalOpen} transparent animationType="slide" onRequestClose={() => setModalOpen(false)}>
        <View style={styles.overlay}>
          <View style={styles.sheet}>
            ...
            <Input label="الفئة (اختياري)" value={category} onChangeText={setCategory} placeholder="مثال: مشروبات، حلويات، ورود" icon="🏷️" />
        ...
        <View style={styles.metaRow}>
          {product.category ? (
            <View style={styles.catChip}>
              <Text style={styles.catChipText} numberOfLines={1}>{product.category}</Text>
            </View>
          ) : null}
...
const styles = StyleSheet.create({
  ...
  gearBtn: { width: 40, height: 40, borderRadius: radii.pill, backgroundColor: colors.surface, alignItems: "center", justifyContent: "center" },
  ...
  metaRow: { flexDirection: "row-reverse", alignItems: "center", gap: spacing.xs, marginTop: 2 },
  catChip: { backgroundColor: colors.surface, paddingHorizontal: spacing.sm, paddingVertical: 2, borderRadius: radii.pill },
  catChipText: { fontSize: fontSize.caption, color: colors.textMuted, fontWeight: fontWeight.semibold, maxWidth: 120 },
